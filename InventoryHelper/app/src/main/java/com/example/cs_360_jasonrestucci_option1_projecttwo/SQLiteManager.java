...
public class SQLiteManager extends SQLiteOpenHelper {
    ...
    private static final String ITEM_NOTES_FIELD = "Notes";
    ...
    public void addItemToDatabase(item item){
        ...
        contentValues.put(ITEM_NOTES_FIELD, item.getNotes());
        ...
        try (Cursor result = sqLiteDatabase.rawQuery("SELECT * FROM " + TABLE_NAME, null)) {
            if (result.getCount() != 0){
                while (result.moveToNext()){
                    ...
                    String notes = result.getString(5);
                    item item = new item(id, name, qty, category, notes);
